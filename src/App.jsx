...
function Menu({page,setPage,introRef}){
  ...
  return (<div id="ukho-menu" style={{...panelStyle,top:0,animation:`menuSlideDown 0.4s cubic-bezier(0.34,1.56,0.64,1) ${delay}ms both`}}>
    ...
    return (<div style={{background:"white",minHeight:"100vh"}}>
      ...
      <div style={{paddingTop:deskTopH+44,paddingBottom:40}}>
        {filtered.map((e,rowIdx)=>(
          <div key={e.id} className="ukho-row" onClick={()=>onOpenEvent?.(e)} style={{animation:`rowWave 0.35s cubic-bezier(0.25,0.8,0.3,1) ${580+Math.min(rowIdx,60)*16}ms both`}}>
...
function PortalsPage(){
  ...
  return(<div style={{minHeight:"100vh",background:"#000",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",paddingTop:HEADER_H-20}}>
    <style>{`@keyframes portalRise{0%{opacity:0;clip-path:inset(100% 0 0 0);filter:blur(16px) brightness(1.3) contrast(0.8)}10%{opacity:1;clip-path:inset(94% 0 0 0);filter:hue-rotate(55deg) blur(13px) contrast(1.4) brightness(1.3)}20%{opacity:0.55;clip-path:inset(82% 0 0 0);filter:hue-rotate(-35deg) blur(11px) contrast(1.2)}30%{opacity:1;clip-path:inset(68% 0 0 0);filter:hue-rotate(25deg) blur(9px) contrast(1.3)}40%{opacity:0.8;clip-path:inset(54% 0 0 0);filter:hue-rotate(-15deg) blur(7px)}52%{opacity:1;clip-path:inset(40% 0 0 0);filter:hue-rotate(18deg) blur(5px) contrast(1.15)}64%{opacity:0.9;clip-path:inset(26% 0 0 0);filter:blur(4px)}75%{opacity:1;clip-path:inset(14% 0 0 0);filter:hue-rotate(-8deg) blur(2px)}86%{opacity:0.96;clip-path:inset(6% 0 0 0);filter:blur(1px)}100%{opacity:1;clip-path:inset(0);filter:blur(0) contrast(1) brightness(1)}}@keyframes portalEdge{0%{opacity:0;top:100%}10%{opacity:0.8;top:94%}20%{opacity:0.5;top:82%}30%{opacity:0.9;top:68%}40%{opacity:0.7;top:54%}52%{opacity:0.85;top:40%}64%{opacity:0.6;top:26%}75%{opacity:0.7;top:14%}86%{opacity:0.4;top:6%}100%{opacity:0;top:0}}`}</style>
    <div style={{position:"relative",width:"min(80vw,500px)",height:"min(80vw,500px)"}}>
      <div ref={mountRef} style={{width:"100%",height:"100%",position:"relative",opacity:loaded?undefined:0,animation:loaded?"portalRise 1s steps(18,end) both":undefined}}/>
      {loaded&&<div style={{position:"absolute",left:0,right:0,height:2,background:"linear-gradient(90deg,transparent,rgba(74,246,38,0.8) 30%,rgba(0,255,213,0.9) 50%,rgba(74,246,38,0.8) 70%,transparent)",boxShadow:"0 0 12px rgba(74,246,38,0.7),0 0 24px rgba(0,255,213,0.4)",pointerEvents:"none",animation:"portalEdge 1s steps(18,end) both",mixBlendMode:"screen"}}/>}
    </div>
    {loaded&&<div style={{fontFamily:ARCH,fontSize:48,color:"rgba(255,255,255,0.15)",letterSpacing:"-1px",marginTop:24,position:"relative",overflow:"hidden",animation:"portalRise 0.7s steps(14,end) 400ms both"}}>under construction<style>{`@keyframes csBlink{0%,90%,100%{opacity:0.5}95%{opacity:0}}`}</style><div style={{position:"absolute",inset:0,background:"rgba(255,60,60,0.6)",animation:"csBlink 4s ease infinite",pointerEvents:"none"}}/></div>}
  </div>);
}
