...
// Single source of truth for splitting an event's tag string. Some tags contain
// a comma and must stay whole, so the Everything list and the search filter both
// go through this rather than each splitting on their own.
const evTags=e=>/let us stay here/i.test(e.t)?[e.t]:e.t.split(',').map(t=>t.trim());
...
function Menu({page,setPage,introRef}){
  ...
  return (<div id="ukho-menu" style={{...panelStyle,top:0,background:"rgba(255,255,255,0.72)",backdropFilter:"blur(40px) saturate(120%)",WebkitBackdropFilter:"blur(40px) saturate(120%)",animation:`menuSlideDown 0.4s cubic-bezier(0.34,1.56,0.64,1) ${delay}ms both`}}>
...
function CardContent({ev,search,selected,showGreen,onClick}){
  ...
  return(<div ref={outerRef} onClick={onClick} style={{
    ...
    <div ref={innerRef} style={{
      ...
      <div data-field="tags" style={{fontFamily:FONT,fontSize:"clamp(12px,2vw,14px)",color:"#000",lineHeight:1.35,textTransform:"uppercase"}}>{hl(ev.t)}</div>
      <div data-field="date" style={{fontFamily:FONT,fontSize:"clamp(12px,2vw,14px)",color:"#000",lineHeight:1.35}}>{ev.d}</div>
    </div>
  </div>);
}
...
function ListPage({events,onOpenEvent,idxRef,searchRef,yearRef,modeRef,scrollRef,progTermsRef,evScrollRef,evSecRef,cameFromEvRef}){
  ...
  const filtered=useMemo(()=>{
    ...
    if(debouncedSearch.trim()){
      if(debouncedProgTerms){
        ...
        const q=norm(debouncedSearch);const evSrc=cameFromEv.current;if(evSrc){const wb=(s,w)=>{const i=s.indexOf(w);if(i<0)return false;const before=i===0||/\W/.test(s[i-1]);const after=i+w.length>=s.length||/\W/.test(s[i+w.length]);return before&&after};const mPe=e=>e.pe.some(p=>wb(norm(p),q)),mPl=e=>wb(norm(e.pl),q),mTag=e=>evTags(e).some(t=>wb(norm(t),q));const scoped=evSrc==="performers"?mPe:evSrc==="places"?mPl:evSrc==="tags"?mTag:null;list=list.filter(scoped||(e=>wb(norm(e.n),q)||mPe(e)||mPl(e)||mTag(e)||e.d.includes(q)||String(e.id)===debouncedSearch))}else{list=list.filter(e=>norm(e.n).includes(q)||e.pe.some(p=>norm(p).includes(q))||e.pr.some(p=>norm(p).includes(q))||norm(e.pl).includes(q)||norm(e.t).includes(q)||e.d.includes(q)||String(e.id).includes(q))}
      }
    }
    return list;
  ...
  const everything=useMemo(()=>({events:reversed.map(e=>e.n).sort(),performers:PERFORMERS,pieces:PROGRAMS,places:[...new Set(reversed.map(e=>e.pl))].sort(),tags:[...new Set(reversed.flatMap(evTags).filter(Boolean))].sort()}),[reversed]);
  ...
  const jumpFrom=useCallback((t,sec)=>{saveEvScroll();setYearFilter("all");setSearch(t);syncCameFromEv(sec||true);setMode("list");setIdx(0);setEnterDir("None")},[]);
  ...
  if(mode==="everything"){const topH=evBarBottom;const items=everything[evSec]||[];const evBase=skipBarIntro?150:500;const restoreScroll=evScrollRef?.current||0;return(<>
    ...
    <div data-scroll-container ref={el=>{if(el&&restoreScroll){requestAnimationFrame(()=>{el.scrollTop=restoreScroll;evScrollRef.current=0})}}} style={{position:"fixed",top:topH,left:0,right:0,bottom:0,overflowY:"auto",WebkitOverflowScrolling:"touch",background:"white",zIndex:1}}>
      <div key={evSec} style={{padding:isDesk?"14px 20px 40px":"14px 14px 40px"}}><div style={{fontFamily:FONT,fontSize:"clamp(13px,2.3vw,16px)",lineHeight:2,color:"#000"}}>
        {items.map((item,i)=>{const useBold=evSec==="performers"||evSec==="pieces";const getSortPos=s=>{if(evSec==="pieces"){const dm=s.match(/\s[—–\-]\s/);if(dm){const first=s.slice(0,dm.index).split(',')[0].trim();const ls=first.lastIndexOf(' ');return ls>=0?ls+1:0}}if(s.startsWith("L'"))return 2;if(s.startsWith("La "))return 3;if(s.startsWith("Ensemble "))return 9;const ls=s.lastIndexOf(' ');if(ls<0)return 0;if(/^(Choir|Ensemble|Orchestra|Quartet|House|Theatre|Ludus|Maniera|Copenhagen|Sforzando|Radar|Stuttgart|Temporis|Soloists|Voice|Armé|Ukraine|Kyiv)$/i.test(s.slice(ls+1)))return 0;if(s===s.toUpperCase())return 0;return ls+1;};const pos=useBold?getSortPos(item):0;const renderItem=useBold?<>{item.slice(0,pos)}<span style={{fontWeight:700}}>{item[pos]}</span>{item.slice(pos+1)}</>:item;return<div key={i} onClick={()=>evSec==="pieces"?jumpFromProgram(item):jumpFrom(item,evSec)} style={{padding:"2px 0",borderBottom:"1px solid rgba(0,0,0,0.025)",cursor:"pointer",textTransform:evSec==="tags"?"uppercase":"none",transition:"transform 0.12s ease,background 0.12s ease",transformOrigin:"left center",animation:`evItemWave 0.3s cubic-bezier(0.25,0.8,0.3,1) ${evBase+Math.min(i,70)*13}ms backwards`}} onMouseEnter={e=>{e.currentTarget.style.background="rgba(74,246,38,0.06)";e.currentTarget.style.transform="scale(0.985)"}} onMouseLeave={e=>{e.currentTarget.style.background="none";e.currentTarget.style.transform="none"}}>{renderItem}</div>})}
      </div></div>
    </div>
  ...
  return (<div data-scroll-container style={{position:"fixed",top:0,left:0,right:0,bottom:0,overflowY:"auto",WebkitOverflowScrolling:"touch"}}>
    ...
    <div style={{position:"relative",zIndex:1,maxWidth:800,margin:"0 auto",padding:"clamp(60px,12vw,120px) clamp(16px,5vw,40px) 80px"}}>
      ...
      <div style={{position:"relative",margin:"60px 0",padding:"20px 0"}}>
        <div style={{fontFamily:ARCH,fontSize:"clamp(135px,34vw,270px)",fontWeight:400,color:"rgba(0,0,0,0.06)",lineHeight:.8,letterSpacing:"-6px",position:"absolute",top:28,left:0,pointerEvents:"none",whiteSpace:"nowrap"}}>UPCOMING</div>
      ...
      {/* Umbrella */}
      <div style={{position:"relative",margin:"60px 0",padding:"20px 0"}}>
        <div style={{fontFamily:ARCH,fontSize:"clamp(135px,34vw,270px)",fontWeight:400,color:"rgba(0,0,0,0.06)",lineHeight:.8,letterSpacing:"-6px",position:"absolute",top:28,left:0,pointerEvents:"none",whiteSpace:"nowrap"}}>UMBRELLA</div>
