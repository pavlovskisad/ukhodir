  ...
  const filtered=useMemo(()=>{
    ...
    if(debouncedSearch.trim()){
      if(debouncedProgTerms){
        ...
        const q=norm(debouncedSearch);const evSrc=cameFromEv.current;if(evSrc){const wb=(s,w)=>{const i=s.indexOf(w);if(i<0)return false;const before=i===0||/\W/.test(s[i-1]);const after=i+w.length>=s.length||/\W/.test(s[i+w.length]);return before&&after};const mPe=e=>e.pe.some(p=>wb(norm(p),q)),mPl=e=>wb(norm(e.pl),q),mTag=e=>e.t.split(',').some(t=>wb(norm(t.trim()),q));const scoped=evSrc==="performers"?mPe:evSrc==="places"?mPl:evSrc==="tags"?mTag:null;list=list.filter(scoped||(e=>wb(norm(e.n),q)||mPe(e)||mPl(e)||mTag(e)||e.d.includes(q)||String(e.id)===debouncedSearch))}else{list=list.filter(e=>norm(e.n).includes(q)||e.pe.some(p=>norm(p).includes(q))||e.pr.some(p=>norm(p).includes(q))||norm(e.pl).includes(q)||norm(e.t).includes(q)||e.d.includes(q)||String(e.id).includes(q))}
      }
    }
    return list;
