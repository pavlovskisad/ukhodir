...
function BottomBar({search,setSearch,onTop,onBottom,onToggleMode,modeLabel,onPrev,onNext,matchIdx,matchCount,years,yearFilter,setYearFilter}){
  ...
  return (<div id="ukho-bar" style={{...panelStyle,top:menuH,bottom:"auto",boxShadow:"0 2px 16px rgba(0,0,0,0.04)",padding:dk?"6px 20px":"4px 12px",display:"flex",flexDirection:"column",gap:dk?6:4}}>
    <div style={{display:"flex",gap:dk?10:6,alignItems:"center",overflow:"hidden"}}>
      <div style={{flex:"1 1 0",position:"relative",minWidth:0,overflow:"hidden"}}>
      ...
      <button onClick={onToggleMode} style={{fontFamily:ARCH,fontSize:dk?42:28,fontWeight:400,padding:dk?"4px 12px":"1px 6px",background:"none",border:`1.5px solid ${GREEN}`,cursor:"pointer",color:"#000",letterSpacing:0,whiteSpace:"nowrap",height:dk?46:34,flexShrink:0,minWidth:dk?180:120,position:"relative",overflow:"hidden",textTransform:"lowercase",display:"flex",alignItems:"center",justifyContent:"center"}}>{modeLabel}<div style={{position:"absolute",inset:0,background:GREEN,animation:"evBlink 1.2s step-end infinite 2s",pointerEvents:"none",opacity:0}}/></button>
...
function ListPage({events,onOpenEvent,idxRef,searchRef,yearRef,modeRef,scrollRef}){
  ...
  if(mode==="everything"){const topH=evBarBottom;const items=everything[evSec]||[];return(<>
    <div data-scroll-container style={{position:"fixed",top:0,left:0,right:0,bottom:0,overflowY:"auto",WebkitOverflowScrolling:"touch",background:"white",zIndex:1}}>
      ...
      <div style={{position:"sticky",top:topH,zIndex:10,background:"rgba(255,255,255,0.92)",backdropFilter:"blur(36px) saturate(150%)",WebkitBackdropFilter:"blur(36px) saturate(150%)",padding:isDesk?"6px 20px":"4px 8px",display:"flex",gap:isDesk?8:4,flexWrap:"nowrap",justifyContent:isDesk?"flex-start":"space-evenly"}}>{Object.keys(everything).map(s=><button key={s} onClick={()=>setEvSec(s)} style={{fontFamily:ARCH,fontSize:isDesk?36:24,fontWeight:400,padding:isDesk?"6px 20px":"4px 10px",flex:isDesk?undefined:"1 1 0",background:evSec===s?"rgba(74,246,38,0.15)":"none",border:"1px solid rgba(0,0,0,0.06)",cursor:"pointer",color:"#000",letterSpacing:"0.5px",textTransform:"lowercase"}}>{s}</button>)}</div>
...
function Home({setPage}){
  ...
  return (<div data-scroll-container style={{position:"fixed",top:0,left:0,right:0,bottom:0,overflowY:"auto",WebkitOverflowScrolling:"touch"}}>
    ...
    <div style={{position:"relative",zIndex:1,maxWidth:800,margin:"0 auto",padding:"clamp(60px,12vw,120px) clamp(16px,5vw,40px) 80px"}}>
      ...
      <div style={{fontFamily:MONO,fontSize:"clamp(11px,2vw,14px)",color:"rgba(0,0,0,0.6)",lineHeight:1.8,letterSpacing:"0px",margin:"32px 0 4px",whiteSpace:"pre-wrap"}}>
        {tw1.displayed}
      ...
      {/* Upcoming */}
      <div style={{position:"relative",margin:"16px 0 48px"}}>
