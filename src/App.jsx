...
function ListPage({events,onOpenEvent,idxRef,searchRef,yearRef,modeRef,scrollRef}){
  ...
  const filtered=useMemo(()=>{
    ...
    if(search.trim()){
      if(progTerms){
        const{raw}=progTerms;const hc=PIECE_EV[raw];if(hc){list=list.filter(e=>e.id===hc)}else{const pn2=s=>norm(s).replace(/\s*[—–\-]\s*/g," - ").replace(/,\s*/g," ").replace(/\s+/g," ").replace(/\s*\d+[''′]\s*$/,"").trim();const splitSep=s=>{for(const sep of[" — "," – ","– ",", "]){const i=s.indexOf(sep);if(i>0)return[s.slice(0,i).trim(),s.slice(i+sep.length).trim()]}return null};const core=s=>{const sp=splitSep(s);if(!sp)return pn2(s);const t=sp[1].split(/\s*[\(\[]/)[0].trim();return pn2(sp[0]+" - "+t)};const names=s=>{const sp=splitSep(s);if(!sp)return[];return sp[0].split(/\s+/).filter(w=>w.length>2).map(w=>norm(w))};const nr=pn2(raw);const cr=core(raw);const rn=names(raw);
        list=list.filter(e=>e.pr.some(p=>{const np=pn2(p);if(np===nr||np.includes(nr)||nr.includes(np))return true;if(core(p)===cr)return true;if(rn.length&&rn.every(w=>np.includes(w))){const sp=splitSep(raw);if(sp){const t=norm(sp[1].split(/\s*[\(\[]/)[0].trim());if(t.length>2&&np.includes(t))return true}}return false}));}
      }else{
  ...
  return (<div data-scroll-container style={{position:"fixed",top:0,left:0,right:0,bottom:0,overflowY:"auto",WebkitOverflowScrolling:"touch"}}>
    ...
    <div style={{position:"relative",zIndex:1,maxWidth:800,margin:"0 auto",padding:"clamp(60px,12vw,120px) clamp(16px,5vw,40px) 80px"}}>
      ...
      <div style={{margin:"40px 0"}}>
        ...
        <TapButton onClick={()=>setPage("cardindex")} style={{fontFamily:ARCH,fontSize:"clamp(72px,18vw,120px)",fontWeight:400,color:BLUE,background:"none",border:"none",cursor:"pointer",padding:"8px 16px",textDecoration:"none",letterSpacing:"-1px",display:"inline-block"}}>
          enter archive
        </TapButton>
      </div>
      ...
      <div style={{position:"relative",margin:"48px 0"}}>
        ...
        <div style={{paddingTop:40,display:"flex",flexDirection:"column",gap:12}}>
          ...
          <TapButton href="https://music.youtube.com/channel/UCOnv8gWUnKFFAC2So6EVS_Q" style={{fontFamily:ARCH,fontSize:"clamp(56px,14vw,88px)",fontWeight:400,color:BLUE,background:"none",border:"none",textDecoration:"none",letterSpacing:"-1px",display:"inline-block",padding:"6px 12px",cursor:"pointer"}}>
            ukho ensemble kyiv
          </TapButton>
        </div>
      </div>
      ...
      <div style={{display:"flex",gap:24,alignItems:"center",margin:"24px 0 40px"}}>
        <TapButton href="https://www.instagram.com/ukho.music/" style={{fontFamily:ARCH,fontSize:"clamp(44px,11vw,64px)",fontWeight:400,color:BLUE,background:"none",border:"none",textDecoration:"none",letterSpacing:"-1px",cursor:"pointer",padding:"4px 8px"}}>inst</TapButton>
        <TapButton href="https://www.facebook.com/ukhomusic" style={{fontFamily:ARCH,fontSize:"clamp(44px,11vw,64px)",fontWeight:400,color:BLUE,background:"none",border:"none",textDecoration:"none",letterSpacing:"-1px",cursor:"pointer",padding:"4px 8px"}}>fb</TapButton>
      </div>
