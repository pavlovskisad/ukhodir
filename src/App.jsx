...
function BottomBar({search,setSearch,onTop,onBottom,onToggleMode,modeLabel,onPrev,onNext,matchIdx,matchCount,years,yearFilter,setYearFilter}){
  ...
  return (<div id="ukho-bar" style={{...panelStyle,top:menuH,bottom:"auto",boxShadow:"0 2px 16px rgba(0,0,0,0.04)",padding:dk?"6px 20px":"4px 12px",display:"flex",flexDirection:"column",gap:dk?6:4}}>
    <div style={{display:"flex",gap:dk?10:6,alignItems:"center",overflow:"hidden"}}>
      ...
      <button onClick={onToggleMode} style={{fontFamily:ARCH,fontSize:dk?42:28,fontWeight:400,padding:dk?"6px 28px":"2px 14px",background:"none",border:`1.5px solid ${GREEN}`,cursor:"pointer",color:"#000",letterSpacing:0,whiteSpace:"nowrap",height:dk?52:38,flexShrink:0,minWidth:dk?180:120,position:"relative",overflow:"hidden",textTransform:"lowercase",display:"flex",alignItems:"center",justifyContent:"center"}}>{modeLabel}<div style={{position:"absolute",inset:0,background:GREEN,animation:"evBlink 1.2s step-end infinite 2s",pointerEvents:"none",opacity:0}}/></button>
...
function CardContent({ev,search,selected,showGreen,onClick}){
  ...
  return(<div ref={outerRef} onClick={onClick} style={{
    ...
    <div ref={innerRef} style={{
      ...
      <div><div data-field="name" style={{fontFamily:ARCH,fontSize:"clamp(14px,2.5vw,18px)",color:"rgba(0,0,0,0.15)",letterSpacing:0,marginBottom:2}}>{ev.id}</div><div style={{fontFamily:ARCH,fontSize:"clamp(36px,8vw,56px)",fontWeight:400,color:"#000",lineHeight:1.15,letterSpacing:"-0.3px",zIndex:1}}>{hl(ev.n)}</div></div>
